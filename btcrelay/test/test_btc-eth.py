from pyethereum import tester

from bitcoin import *


import datetime
import struct
import pytest
slow = pytest.mark.slow

class TestBtcEth(object):

    CONTRACT = 'btc-eth.py'
    CONTRACT_GAS = 55000

    NAME_REG = 'namereg.se'

    ETHER = 10 ** 18

    def _storage(self, contract, idx):
        return self.s.block.account_to_dict(contract.address)['storage'].get(idx)

    def setup_class(cls):
        cls.s = tester.state()
        cls.c = cls.s.abi_contract(cls.CONTRACT, endowment=2000*cls.ETHER)
        cls.snapshot = cls.s.snapshot()
        cls.seed = tester.seed

    def setup_method(self, method):
        self.s.revert(self.snapshot)
        tester.seed = self.seed


    def testTransfer(self):
        self.namereg = self.s.abi_contract(self.NAME_REG)
        ans = self.namereg.register(self.c.address, "BtcRelay")
        assert ans == 1
        assert self._storage(self.namereg, "0x" + self.c.address) == "0x" + "BtcRelay".encode('hex')


        # tx is fff2525b8931402dd09222c50775608f75787bd2b87e56995a7bdd30f79702c4
        txStr = ''
        res = self.c.processTransfer(txStr, profiling=True)
        print('GAS: '+str(res['gas']))
        assert(res['output'] == 1)

        expEtherAddr = '948c765a6914d43f2a7ac177da2c2f6b52de3d7c'
        userEthBalance = self.s.block.get_balance(expEtherAddr)
        print('USER ETH BALANCE: '+str(userEthBalance))
        expEtherBalance = 13
        assert userEthBalance == expEtherBalance
